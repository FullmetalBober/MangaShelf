...
class GiveToPage:
    ...
    Catalogue = [{'title': manga['title'], 'description': manga['description'], 'image': manga['image']} for manga
                 in
    ...
    def reloadCatalogue(page=CataloguePage, update=False):
        ...
        if save:
            ...
            GiveToPage.Catalogue = [
                {'title': manga['title'], 'description': manga['description'], 'image': manga['image']}
                for manga in app.db.Catalogue.find({}).skip((page - 1) * GiveToPage.CataloguePageCount).limit(
                    GiveToPage.CataloguePageCount)]
    ...
    def get_dictionary():
        return {'Catalogue': GiveToPage.Catalogue, 'User': GiveToPage.User, 'UserLogin': GiveToPage.UserLogin,
                'CataloguePages': GiveToPage.CataloguePages, 'CataloguePage': GiveToPage.CataloguePage}
